...
public interface DepensesTicketRepository extends JpaRepository<DepensesTicket, Integer> {
    @Query("SELECT d from DepensesTicket d order by d.dateDepense")
    ...
    @Query("SELECT COALESCE(SUM(dl.amount), 0) FROM DepensesTicket dl WHERE dl.ticket.customer.id = :idCustomer AND dl.dateDepense <= :date")
